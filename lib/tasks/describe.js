        ...
        for (const key of packageListNames) {
            ...
            for (const dependency of dependenciesNames) {
                if (descriptions[dependency].status === 'unknown') {
                    const defaultDescription = defaultDescriptions[dependency];
                    const currentDescription = descriptions[dependency];

                    const description = await getDescription(
                        dependency,
                        currentDescription.status === 'unknown' && defaultDescription
                            ? defaultDescription
                            : currentDescription,
                    );
